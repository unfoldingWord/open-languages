...
DECLARE
    result_json jsonb;
    all_access_statuses jsonb[]; -- Change the data type to jsonb array
BEGIN
    -- Aggregate all_access_status values into an array of JSON objects
     SELECT ARRAY_AGG(
        JSONB_BUILD_OBJECT(
            'all_access_status', all_access_status,
            'first_language_pop', first_language_pop
        )
    ) INTO all_access_statuses
